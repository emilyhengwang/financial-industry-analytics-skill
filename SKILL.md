...
description: >
  Tutor skill for MSBA Financial Industry Analytics students. Use for finance concept questions
  (returns, volatility, GARCH, VaR, CVaR, Sharpe ratio, CAPM, Fama-French, efficient frontier,
  mean-variance optimization, min-variance, max-Sharpe, FinBERT, sentiment analysis),
  Python debugging (pandas, yfinance, numpy, scipy, cvxpy, statsmodels, transformers),
  step-by-step coding walkthroughs, or connecting code to financial meaning.
  Trigger whenever a student seems confused about finance theory, Python errors, portfolio
  optimization, asset pricing, time series, risk analytics, or NLP for finance —
  especially for students without a finance or coding background.
---
